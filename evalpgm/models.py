    ...
    def __init__(
        ...
        self.encoder = nn.Sequential(
            ...
            *[ResidualBlock(hidden_sizes[2], kernel_size= 5, dropout = 0.2) for _ in range(blocks_per_stage)],
            ...
            Permute(0,2,1),
            *[TransformerRoPE(hidden_sizes[3], n_heads, dropout = 0.2, beta_swi=beta_swi, activation=activation) for _ in range(blocks_per_stage)],
            Permute(0,2,1),
            nn.Conv1d(hidden_sizes[3], hidden_sizes[4], kernel_size = 4, stride = 4), # 16 -> 4
            Permute(0,2,1),
            *[TransformerRoPE(hidden_sizes[4], n_heads, dropout = 0.2, beta_swi=beta_swi, activation=activation) for _ in range(blocks_per_stage)],
            Permute(0,2,1),
